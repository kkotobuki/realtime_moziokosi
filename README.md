# リアルタイム文字起こし with マーメイド記法自動生成 (統合版)

Next.js単一プロジェクトで構成されたリアルタイム音声認識と文字起こし履歴から自動でマーメイド記法を生成するシステムです。
...
- **マーメイド記法自動生成**: Gemini APIを活用した文字起こし内容からの図解化
...
│   └── api/
│       └── mermaid/
│           └── route.ts       # マーメイド記法生成API
├── components/
│   └── MermaidViewer.tsx      # マーメイド記法表示
├── lib/                        # バックエンドロジック
...
4. 左側に文字起こし履歴が蓄積されます
5. 「マーメイド記法を生成」ボタンで履歴からマーメイド記法を生成
...
Next.js App RouterのAPI Routesを使用して、マーメイド記法生成などのREST APIを提供しています。
